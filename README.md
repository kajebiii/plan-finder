...
| `--effort` | | Claude 세션의 reasoning effort: `low` / `medium` / `high` / `xhigh` / `max`. 다른 값은 typer 가 즉시 거부. 적용되면 시작 시 `Reasoning effort: <level>` 로 한 줄 출력. `claude --effort <level>` 로 패스스루. Codex 백엔드면 무시 | (Claude CLI 기본값) |
